 ...
 while(!q.empty())
   {
     ...
     if(front->up == NULL)
       ...
       cout << front->el << " " << front->BF << " " << front->up->el << "->";
     q.pop();
...
{
  Node<T>* bal = updateBF(newNode);
  if(bal == NULL)//if returns NULL, no need to balance the tree
    {
      return;
    }
  else if(bal != NULL) //not balanced
    
    {
      Node<T>* parent = bal->up;
      Node<T>* temp;
      bool rightChild; 
	...
	{
	  if(parent->right == bal)
	    rightChild = true;//violation is found on the right
	  else
	    rightChild = false;//violation is found on the left
	}
      
      if(bal->BF == 2 && bal->left != NULL && bal->left->BF == 1)
	{
	  bal = rightRotation(bal);
	}
      else if(bal->BF == 2 && bal->left != NULL && bal->left->BF == -1)//LR violation
	{
	  temp = leftRotation(bal->left);
	  temp->up = bal;
	  bal->left = temp;
	  bal = rightRotation(bal);
	}
      else if(bal->BF == -2 && bal->right != NULL && bal->right->BF == 1)//RL
	{
	  
	  temp = rightRotation(bal->right);
	  temp->up = bal;
	  bal->right = temp;
	  bal = leftRotation(bal);
	}
      else if(bal->BF == -2 && bal->right != NULL && bal->right->BF == -1)//RR
	{
	  bal = leftRotation(bal);
	}
